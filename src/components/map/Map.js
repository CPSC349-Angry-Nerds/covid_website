...
const MapChart = () => {
  ...
  const handleShow = async (state) => {
    ...
    const data = await fetch(`https://api.covidtracking.com/v1/states/${state.toLowerCase()}/daily.json`).then( res => res.json());
    const dateArr = [];
    const deathArr = [];
    const posArr = [];
    const currArr = [];
  ...
  const data2 = {
    ...
    <ComposableMap projection="geoAlbersUsa">
                  ...
                  {cur &&
                    ...
                    (Object.keys(offsets).indexOf(cur.id) === -1 ? (
                      <Marker coordinates={centroid}>
                        <text y="2" onClick={() => handleShow(cur.id) } className="pointer" fontSize={14} textAnchor="middle">
                          {cur.id}
                        </text>
                      </Marker>
                    ) : (
                      <Annotation
                        ...
                        dy={offsets[cur.id][1]}
                      >
                        <text x={4} onClick={() => handleShow(cur.id)} className="pointer" fontSize={14} alignmentBaseline="middle">
                          {cur.id}
                        </text>
                      </Annotation>
                    ))}
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body><Line options={options} data={data} />
